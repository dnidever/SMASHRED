...
pro smashred_load_catphot_healpix,pix,info,chstr,allsrc,useast=useast,useorig=useorig,reduxdir=reduxdir,redo=redo,error=error,$
                                  outputdir=outputdir,buffer=buffer
...
if n_elements(info) eq 0 then begin
  ...
  print,'Syntax - smashred_load_catphot_healpix,pix,info,chstr,allsrc,useast=useast,useorig=useorig,reduxdir=reduxdir,redo=redo,error=error'
  return
endif
...
; Output filename
outfile = tmpdir+strtrim(pix,2)+'_'+fbase+'_'+info.night+'_photred.fits'
...
allsrc_schema = {cmbindx:-1L,chipindx:-1L,fid:'',id:-1L,idref:-1L,x:0.0,y:0.0,xref:0.0,yref:0.0,forced:0B,$
                 mag:0.0,err:0.0,cmag:-1.0,cerr:-1.0,chi:0.0,sharp:0.0,flag:-1,prob:-1.0,ra:0.0d0,dec:0.0d0,$
...
If file_test(outfile) eq 0 or keyword_set(redo) then begin
  ...
  for i=0,nphotfiles-1 do begin  
    ...
    for k=0,nchind-1 do begin
      ...
      nx = sxpar(hd1,'naxis1')
      ny = sxpar(hd1,'naxis2')
      HEAD_XYAD,hd1,[0,nx-1,nx-1,0],[0,0,ny-1,ny-1],vra,vdec,/deg
      ROTSPHCEN,vra,vdec,buffer.cenra,buffer.cendec,lon,lat,/gnomic
      chstr[chind[k]].vertices_ra = vra
      chstr[chind[k]].vertices_dec = vdec
      chstr[chind[k]].overlap = dopolygonsoverlap(lon,lat,buffer.lon,buffer.lat)
    endfor
    if total(chstr[chind].overlap) eq 0 then begin
    ...
    ;;; Only keep sources INSIDE the boundary
    ;ROTSPHCEN,phot.ra,phot.dec,buffer.cenra,buffer.cendec,lon,lat,/gnomic
    ;ROI_CUT,buffer.lon,buffer.lat,lon,lat,ind,cutind,fac=1000,/silent
    ;nmatch = n_elements(cutind)
    ;if nmatch eq 0 then dum=where(cutind ge 0,nmatch)
    ...
    for j=0,nchind-1 do begin
      ...
      if nsrc eq 0 then begin
        print,'No good sources in this chip'
        gaiarms = 999999.0
        gaianmatch = 0
        gaiahead = head
        undefine,src
      ...
      if tag_exist(phot,'PROB') then src.forced=1 else src.forced=0
