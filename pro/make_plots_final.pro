...
;for i=38,nfiles-1 do begin
  ...
  file1 = outdir+field+'_hess_final'+version
  file2 = outdir+field+'_hess_final'+version+'_dered'
...
;  endif
  ...
  ;setdisp
  !p.font = 0
  file = outdir+field+'_hess_final'+version
 ...
 ; endif
  ...
  ;setdisp
  !p.font = 0
  file = outdir+field+'_hess_final'+version+'_dered'
  if file_test(file+'.eps') eq 0 or file_test(file+'.jpg') eq 0 or keyword_set(redo) then begin
  ;if file_test(file+'.eps') eq 0 or keyword_set(redo) then begin
    ps_open,file,/color,thick=4,/encap
    ;device,/inches,xsize=8.5,ysize=10
