function gsmooth,arr,fwhm,widfwhm=widfwhm0,psf=psf,stp=stp,squared=squared,error=error
...
if (sz[0] eq 1) then begin
  ...
  ;; need an odd number for the convolution to be correct
  if npix mod 2 eq 0 then if npix eq narr then npix-- else npix++
...
if (sz[0] eq 2) then begin
  ...
  if fw[0] gt 0 and fw[1] eq 0 then begin
    ...
    if npix mod 2 eq 0 then if npix eq sz[1] then npix-- else npix++
    if npix eq 0 then return,arr   ; nothing to smooth
  ...
  if fw[0] eq 0 and fw[1] gt 0 then begin
    ...
    if npix mod 2 eq 0 then if npix eq sz[2] then npix-- else npix++
